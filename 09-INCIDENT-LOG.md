...
| 026 | 12 | CI had never once gone green, on any commit, since the workflow file was first added — the badge in the README was aspirational the entire build. Root cause: a job-level `if: ${{ secrets.ANTHROPIC_API_KEY != '' }}` on the `llm-eval` job. GitHub Actions doesn't expose the `secrets` context to a job-level `if:` ("Unrecognized named-value: 'secrets'"), and that single invalid expression fails YAML validation for the **entire workflow file** — every job on every run, confirmed via the API returning `total_count: 0` jobs on every single run in the repo's history. Fixing it exposed three more real bugs the invalid workflow had been hiding: (a) `ruff-pre-commit` pinned to `v0.8.4` while `pyproject.toml` left the actual `ruff` dependency unbounded, so `uv sync` always installed the latest (0.16.5) and the two disagreed on formatting; (b) `docker/Dockerfile.api` — used by both the `api` and `worker` services — is bare `python:3.12-slim` with no system packages, and LightGBM's compiled library needs `libgomp` (GNU OpenMP), absent from that image, so the deployed worker would crash with `OSError: libgomp.so.1` the instant it classified a real case; (c) the `tests` job ran pytest without ever training a model first, so any test touching `ml/artifacts/classifier/model.joblib` failed on a genuinely fresh runner — invisible locally only because a prior `make train` had already left the artifact sitting there | 2.3h | fixed all four: moved the LLM-eval gate inside the step body (checked via `env`, not `if:`); pinned ruff to `0.16.5` in both places; added `apt-get install libgomp1` to the Dockerfile; added the same three training commands the `models` job already ran, before `tests`'s first pytest step; also upgraded `gitleaks-action` v2→v3 with a full (non-shallow) checkout and an explicit `GITHUB_TOKEN`, since v2's breaking change silently required it. Diagnosed by reproducing CI's exact commands in a matching Linux container (Windows passed clean throughout, masking the platform-specific parts) once the real pytest traceback was available. All 4 jobs (`quality`, `tests`, `models`, `llm-eval`) confirmed green on `main` afterward |
| 027 | 12 | `docker compose up` had never once produced a working `api` or `worker` container — nobody had run it end to end since `docker/Dockerfile.api` was written. Six independent, stacked failures, each one hiding the next until the previous was fixed: (1) INC-026's `libgomp1` gap; (2) `pyproject.toml` declares `readme = "README.md"` for the hatchling build backend, but the Dockerfile never `COPY`s it in, so `uv sync`'s final install step fails outright; (3) `uv sync --no-dev` (no `--all-extras`) skips the `ml` dependency group, and `measurement/cuped.py` imports `scipy` at module level — a path the live API hits on every dashboard request, not just `make train` — so the container crashed on import; (4) `policies/` is never copied in, and `execution/templates.py` reads `policies/templates/recovery.yaml` at *import* time (triggered transitively by `api/dashboard.py` → `chaos/scenarios.py`), so the app failed before serving a single request; (5) `ml/artifacts/**/*.joblib` is correctly gitignored as a build artifact, so a freshly-built image has no trained models at all, and `understanding/classify.py` needs one; (6) no service in `docker-compose.yml` ever ran `alembic upgrade head` — a truly fresh Postgres would have zero tables — and worse, `api`/`worker` inherited `.env`'s `DATABASE_URL=...@localhost:5434/...`, which is correct for host-side tooling but meaningless inside a container (`localhost` there means the container itself, not the host) | 1.6h | fixed all six: `COPY README.md`; `uv sync --no-dev --all-extras` (matching CI); `COPY policies`; `COPY ml` plus running the same three training commands during the image build so the container is self-contained; added a one-shot `migrate` service (`alembic upgrade head`, `restart: "no"`) that `api`/`worker` depend on via `condition: service_completed_successfully`; added an `environment:` override (via a YAML anchor, `environment:` wins over `env_file:` on a key collision) pointing both at the compose network's `postgres`/`redis` hostnames instead of the host-oriented `.env` values. Verified for real, not assumed: full `docker compose up -d --build migrate api worker` from a clean state, confirmed `migrate` exits 0, both containers stay `Up` rather than crash-looping, and `curl` against the containerized API returns real data from the real database |
| 028 | 12 | The Approval Queue panel had been silently empty *the entire session* — INC-021's exact sibling bug, in a file that fix never touched. `GET /approvals` called `ApprovalStore.list_pending(policy.merchant.merchant_id)`, filtering on `"demo"` (the policy config's own label); the actual rows are written with `case.merchant_id` — the real business-profile label (`demo-b2b`, etc.) — at write time. Every approval request was silently invisible to its own read path. Found while working out how to reproduce `07-DEMO-SCRIPT.md`'s "approve a card, then cancel it" beat and discovering the queue had nothing in it — a direct query against `pending_approvals` showed 582 real rows sitting under `demo-b2b`, matched by zero rows under `"demo"` | 0.3h | dropped the merchant_id filter from `list_pending()` (single-tenant build, same fix as INC-021); confirmed live — the queue went from 0 to 583 real approvals the moment the fix deployed. Also added `scripts/mint_demo_link.py`, a small helper to mint a valid single-use recovery-page token for any case on demand, since the video's "customer side" beat needs a real, unexpired link and the ones already sent during `make demo` are long since redeemed or expired; verified end to end — minted a link, opened it, simulated the payment, watched the case actually drop off the live work queue |
...
**Total time lost to incidents:** 15.2h
